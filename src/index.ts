...
export class Heroku {
  ...
  private async _fetch(props: {route: string, method?: RequestInit['method'], body?: RequestInit['body']}) {
    const { route, method = 'GET', body } = props
    return await fetch(`https://api.heroku.com${route}`, {
      method,
      body,
      headers: {
        ...['PATCH', 'POST', 'PUT'].includes(method) && { 'Content-Type': 'application/json' },
  ...
  async updateAppBuildpacks(props: { 
    appName: string; buildpacks: string[]
  }): Promise<boolean> {
    const { appName, buildpacks } = props
    return (await this._fetch({
      route: `/apps/${appName}/buildpack-installations`,
      method: 'PUT',
      body: JSON.stringify({ updates: buildpacks.map(buildpack => ({ buildpack })) })
    })).ok
  }

  async createApp(props: { 
    appName: string, region?: string, team?: string 
  }): Promise<App> {
  ...
  private async _getPipeline(props: { 
    ...
    const doc = await (await this._fetch({ route: `/pipelines/${pipelineName}` })).json()
    return doc.id === 'not_found' ? undefined : doc
  }

  async addAppToPipeline(props: { 
    appName: string, pipelineName: string, 
  ...
  async getAppEnvVars(props: {
    ...
    const { appName } = props
    return await (await this._fetch({ route: `/apps/${appName}/config-vars` })).json()
  }
  ...
  async enableAppAutoCerts(props: {
    appName: string
  }): Promise<boolean> {
    const { appName } = props
    return (await this._fetch({ route: `/apps/${appName}/acm`, method: 'POST' })).ok
  }
}
